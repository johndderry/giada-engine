		...
		case FL_PUSH:	{
			...
			if (Fl::event_button1()) {
				...
				if (selected == NULL) {
					...
					add(a);
					mainWin->launcher->setChannelWithActions((gSampleChannel*)ch->guiChannel); // mainWindow update
					redraw();
					ret = 1;
				}
				else {
			...
			if (Fl::event_button3()) {
				...
				if (a != NULL) {
					...
					remove(a);
					delete a;
					mainWin->launcher->setChannelWithActions((gSampleChannel*)pParent->chan->guiChannel);
					redraw();
					ret = 1;
				}
			}
