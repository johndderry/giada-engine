	...
	: Fl_Group(X, Y, W, H, "Misc")
{
	...
	useVerify = new gCheck(x(),  debugMsg->y()+26,  40, 20, "Use confirmation dialog");
	useAltClick = new gCheck(x(),  useVerify->y()+26,  40, 20, "Use alternate click style");
	useMidiClick = new gCheck(x(),  useAltClick->y()+26,  40, 20, "Use MIDI click on Chan 10");
	end();	
	...
	if( G_Conf.useVerifyDialog ) useVerify->value(1);
	if( G_Conf.useAltClick ) useAltClick->value(1);
	if( G_Conf.useMidiClick ) useMidiClick->value(1);
...
void gTabMisc::save()
{
	...
	if( useVerify->value() ) G_Conf.useVerifyDialog = true;
	else					 G_Conf.useVerifyDialog = false;
	if( useAltClick->value() )	G_Conf.useAltClick = true;
	else						G_Conf.useAltClick = false;
	if( useMidiClick->value() )	G_Conf.useMidiClick = true;
	else						G_Conf.useMidiClick = false;
}
	...
	: Fl_Group(X, Y, W, H, "MIDI")
{
	...
	tackMesg = new gValue(x()+122, h()-130, 60, 20, "Tack Message");
	tickMesg = new gValue(x()+122, h()-100, 60, 20, "Tick Message");
	tockMesg = new gValue(x()+122, h()-70, 60, 20, "Tock Message");

	new gBox(x(), sync->y()+sync->h()+8, w(), h()-50, "Restart Giada for the changes to take effect.");
	end();
	...
	tackMesg->value( G_Conf.midiTackMesg );
	tickMesg->value( G_Conf.midiTickMesg );
	tockMesg->value( G_Conf.midiTockMesg );
...
void gTabMidi::save() 
{
	...
	G_Conf.midiTackMesg = tackMesg->value();
	G_Conf.midiTickMesg = tickMesg->value();
	G_Conf.midiTockMesg = tockMesg->value();
}
