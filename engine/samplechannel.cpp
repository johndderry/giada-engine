//
// C++ Implementation: samplechannel
//
// Description: 
//
...
void SampleChannel::parseAction(void *a, int localFrame, int globalFrame) {

	if (readActions == false)
		return;

	switch (((recorder::action *)a)->type) {
...
bool SampleChannel::allocEmpty(int frames, int takeId) {
	...
	/* during allocation of the channel we use the logical name
	 * and write it out to the pathfile, name of Channel::. This
	 * is not however, where the logical name is incremented. 
	 * That is occurring in G_Mixer, when the sample channel is 
	 * actually being recorded. How does that work?
	 */
	...
	pathfile    = gGetCurrentPath()+"/"+wname;
	name        = wname;
	wave        = w;
	status      = STATUS_OFF;
	begin       = 0;
...
int SampleChannel::load(const char *file) {
	...
	pathfile = file;
	name     = gStripExt(gBasename(file).c_str());
	...
	int k = 1;
	while (!mh_uniqueSamplename(this, name.c_str())) {
		updateName((oldName + "-" + gItoa(k)).c_str());
		k++;
	}
