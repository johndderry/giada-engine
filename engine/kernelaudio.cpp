		...
		void *arg)
{
	switch (state) {
		case JackTransportStopped:
			...
			control::stopSeq(false);  // false = not from GUI
			if (pos->frame == 0)
				control::rewindSeq();
			break;
		...
		case JackTransportStarting:
			...
			control::startSeq(false);  // false = not from GUI
			if (pos->frame == 0)
				control::rewindSeq();
			break;
