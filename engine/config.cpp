...
std::string Conf::getValue(const char *in) {
...
void Conf::setDefault()
{
  ...
  sampleEndMode = 1;
	...
	midiTockMesg   = 0x2a40;	// closed hihat, midi count
...
int Conf::read()
{
	...
	useVerifyDialog    = atoi(getValue("useVerifyDialog").c_str());
	useAltClick        = atoi(getValue("useAltClick").c_str());
	useMidiClick       = atoi(getValue("useMidiClick").c_str());
	incrMidiChan       = atoi(getValue("incrMidiChan").c_str());
	autoPlayMidi       = atoi(getValue("autoPlayMidi").c_str());
...
int Conf::write()
{
	...
	fprintf(fp, "useVerifyDialog=%d\n",   useVerifyDialog);
	fprintf(fp, "useAltClick=%d\n",   useAltClick);
	fprintf(fp, "useMidiClick=%d\n",   useMidiClick);
	fprintf(fp, "incrMidiChan=%d\n",   incrMidiChan);
	fprintf(fp, "autoPlayMidi=%d\n",   autoPlayMidi);
